...
/**
 * Panneau de contrôle PixelLife — v3.
 *
 * Changements :
 *  - Ratios remplacés par des comptages absolus (plantCount / nutrientCount / organismCount)
 *  - maxSteps = 0 → "INFINI" (label adaptatif dans le spinner)
 *  - Spinner mutationRate ajouté
 *  - Badge "∞" dans le LCD step quand la simulation est infinie
 */
public class ControlPanel extends JPanel {
    ...
    private JLabel   lblMaxStepsHint;  // affiche "INFINI" quand value = 0
    ...
    private JPanel buildControlBlock() {
        ...
        togglePause.addItemListener(e -> {
            ...
            if (togglePause.isSelected()) {
                ...
                lcdStatus.setText("PAUSED");
                lcdStatus.setForeground(LCD_AMBER);
            } else {
                client.sendResume(currentSimId);
                client.sendSetSpeed(currentSimId, speedSlider.getValue());
                lcdStatus.setText("RUNNING");
                lcdStatus.setForeground(LCD_GREEN);
            }
        });
    ...
    private void onStop() {
        ...
        simRunning = false;
        lcdStatus.setText("STOPPED");
        lcdStatus.setForeground(LCD_RED);
        syncButtons();
    }
    ...
    private void randomizeConfig() {
        ...
        // 0 ou un nombre aléatoire (50 % infini)
    ...
    public void notifySimulationEnded() {
        SwingUtilities.invokeLater(() -> {
            simRunning = false;
            lcdStatus.setText("FINISHED");
            lcdStatus.setForeground(new Color(0x888888));
            syncButtons();
        });
    }
    ...
    private String buildJson() {
        return String.format(Locale.US,
                "{\"config\":{\"width\":%d,\"height\":%d," +
                        ...
                        "\"maxSteps\":%d,\"seed\":%d,\"mutationRate\":%.2f}}",
                (int) spinWidth.getValue(),
                (int) spinHeight.getValue(),
                (int) spinPlantCount.getValue(),
                (int) spinNutrientCount.getValue(),
                (int) spinOrganismCount.getValue(),
                (int) spinMaxSteps.getValue(),
                (int) spinSeed.getValue(),
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // Composants custom (identiques à v2 — SteamButton, ToggleSwitch, etc.)
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    private static class SteamButton extends JButton {
        ...
        @Override public Dimension getPreferredSize(){ return new Dimension(super.getPreferredSize().width,40); }
    }

    private static class ToggleSwitch extends JToggleButton {
        ToggleSwitch(String label) {
            super(label); setFont(BTN_FONT); setForeground(COPPER_LIGHT);
            setFocusPainted(false); setBorderPainted(false); setOpaque(false);
        ...
        @Override public Dimension getPreferredSize(){ return new Dimension(getParent()!=null?getParent().getWidth()-80:120,46); }
    }
    ...
    private static class RivetBorder extends AbstractBorder {
        @Override public void paintBorder(Component c,Graphics g,int x,int y,int w,int h){
            ...
            int r=4;
            drawRivet(g2,x+r+2,y+r+2); drawRivet(g2,x+w-r-3,y+r+2);
            drawRivet(g2,x+r+2,y+h-r-3); drawRivet(g2,x+w-r-3,y+h-r-3);
            g2.dispose();
        }
